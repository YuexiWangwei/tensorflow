...
def imageprepare(argv): # 该函数读一张图片，处理后返回一个数组，进到网络中预测
    ...
    if width > height:  # check which dimension is bigger
        ...
        if (nwidth == 0):  # rare case but minimum is 1 pixel
            nwidth = 1
            # resize and sharpen
        img = im.resize((nwidth, 20), Image.ANTIALIAS).filter(ImageFilter.SHARPEN)  #高质量填充并进行锐化
        wleft = int(round(((28 - nwidth) / 2), 0))  # caculate vertical pozition
        newImage.paste(img, (wleft, 4))  # paste resized image on white canvas

    # newImage.save("sample88.png")
...
with tf.Session(graph=myGraph) as sess:
    ...
    array = imageprepare('./8.png') # 读一张包含数字的图片

    result = sess.run(y_end,feed_dict={x_raw:[array],keep_prob:1.0})
    print(result)

    prediction = tf.argmax(y_conv, 1)  # 预测,返回的是最大的序号
